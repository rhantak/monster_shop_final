...
RSpec.describe Order do
  ...
  describe 'class methods' do
    before :each do
      @user = User.create!(name: 'Megan', email: 'megan_1@example.com', password: 'securepassword')
      @address_1 = Address.create(nickname: 'Home', name: 'Megan', street_address: '123 A Street', city: 'Dallas', state: 'TX', zip: '75070', user_id: @user.id)
      @order_1 = @user.orders.create!(status: 1, address_id: @address_1.id)
      @order_2 = @user.orders.create!(status: 0, address_id: @address_1.id)
      @order_3 = @user.orders.create!(status: 3, address_id: @address_1.id)
      @order_4 = @user.orders.create!(status: 2, address_id: @address_1.id)
    end
